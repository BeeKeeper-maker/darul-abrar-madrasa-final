...
@section('header', 'নতুন নোটিশ তৈরি করুন')
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="card fade-in-up">
            <div class="card-header">
                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">নোটিশের তথ্য</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('notices.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" rows="6" 
                                  class="form-textarea @error('description') border-danger-300 @enderror" required>{{ old('description') }}</textarea>
                        @error('description')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="notice_for" name="notice_for" class="form-select @error('notice_for') border-danger-300 @enderror" required>
                            ...
                            <option value="all" {{ old('notice_for') == 'all' ? 'selected' : '' }}>সবার জন্য</option>
                            <option value="students" {{ old('notice_for') == 'students' ? 'selected' : '' }}>ছাত্র-ছাত্রীদের জন্য</option>
                            <option value="teachers" {{ old('notice_for') == 'teachers' ? 'selected' : '' }}>শিক্ষকদের জন্য</option>
                            <option value="staff" {{ old('notice_for') == 'staff' ? 'selected' : '' }}>স্টাফদের জন্য</option>
                            <option value="guardians" {{ old('notice_for') == 'guardians' ? 'selected' : '' }}>অভিভাবকদের জন্য</option>
                        </select>
                        @error('notice_for')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex justify-end space-x-4 pt-6">
                        ...
                        <button type="submit" class="btn btn-primary">নোটিশ তৈরি করুন</button>
                    </div>
                </form>
            </div>
        </div>
